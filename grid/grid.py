...
class Grid:
    ...
    def _redraw(self):
        ...
        for i in range(dimension):
            for j in range(dimension):
                ...
                if self.raster.selected_cells[i][j]:  
                    color = 'red'
                elif self.raster.colors[i][j] is not None:
                    color = self.raster.colors[i][j]
                elif self.raster.rendered_cells[i][j]:
                    color = '#444444'
                else:
                    color = '' 
                ...
                text_color = 'white' if color == 'red' else ''
                if self.raster.selected_cells[i][j]:
                    text = str(self.raster.selected_cells[i][j])  
                    x_center = (x1 + x2) / 2
                    y_center = (y1 + y2) / 2
    ...
    def color_cell(self, cell, color='#0019ba'):
        if self.raster._cell_is_in_bounds(cell):
            x, y = self.raster.coordinate_to_index(cell)
            self.raster.rendered_cells[x][y] = 1  
            self.raster.set_cell_color(cell, color)

            x1 = x * self.cell_size[0] + Grid.MARGIN_SIZE
            y1 = (2 * self.raster.extent - y) * self.cell_size[1] + Grid.MARGIN_SIZE
            x2 = x1 + self.cell_size[0]
            y2 = y1 + self.cell_size[1]

            self.canvas.create_rectangle(x1, y1, x2, y2, fill=color, outline='#0019ba')
        else:
            print(f'WARNING: color_cell command ignored. Cell {cell} is out of bounds.')
